...
function populateReplyForm(url,options,err){if(options&&options["reply-placement"]){if(options["reply-placement"]=="above"){$("#comments").prepend("<div id=\"comments-reply\"></div>");}
...
");}
if(err){$("#comments-reply").prepend("<h2>Comment Rejected</h2><blockquote><p>Your comment was not submitted: "+err+"</p></blockquote>");}
$("#ecommentsreply").bind("submit",(function(){$("#ecommentsreply").hide();$.ajax({type:"POST",url:url,data:$("#ecommentsreply").serialize(),error:function(r,e){var err=false;if(r.status==500)
{err=r.responseText;}
loadComments(url,options,err);},complete:function(r,e){var err=false;if(r.status==500)
{err=r.responseText;}
loadComments(url,options,err);},datatype:'jsonp',});return false;}));}
